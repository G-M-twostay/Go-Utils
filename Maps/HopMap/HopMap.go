...
func New[K constraints.Integer, V any](h byte) *HopMap[K, V] {
	return &HopMap[K, V]{make([]Element[K, V], 4), h, maphash.MakeSeed()}
}
...
func (u *HopMap[K, V]) mod(index int) int {
...
func (u *HopMap[K, V]) Get(key K) (V, bool) {
	if i0 := u.mod(int(u.hash(key))); u.bkt[i0].get(hashed) {
		for i1 := u.mod(i0 + int(u.bkt[i0].hashOS)); ; i1 = u.mod(i1 + int(u.bkt[i1].linkOS)) {
...
func (u *HopMap[K, V]) within(v, high int) bool {
	if low := u.mod(high - int(u.H)); high >= low {
		return high > v && v > low
	} else {
		return v > low || v < high
	}
...
func (u *HopMap[K, V]) fillEmpty(i_hash int, i_empty int, k *K, v *V) {
	...
	if u.bkt[i_hash].get(hashed) { //something else already hashed to i_hash, chain it to linked list
		i0 := u.mod(i_hash + int(u.bkt[i_hash].hashOS))
		for ; u.bkt[i0].get(linked); i0 = u.mod(i0 + int(u.bkt[i0].linkOS)) {
...
func (u *HopMap[K, V]) moveAndPut(i_empty int) int {
	for i := u.mod(i_empty - int(u.H) + 1); i != i_empty; i = u.mod(i + 1) { //iterate from i_hash to i_empty
			...
			for i1 := u.mod(i0 + int(u.bkt[i0].hashOS)); ; i1 = u.mod(i1 + int(u.bkt[i1].linkOS)) {
				if u.within(i1, i_empty) { //a value e1 with hash i is located in [i_empty-H,i_empty); so we swap e1 with e_empty
					...
					*prev = int16(i_empty - i0)
					...
					//if u.mod(i_empty-i_hash) < int(u.H) { //i_empty is now within i_hash+H
...
func (u *HopMap[K, V]) Put(key K, val V) {
	i_hash := u.mod(int(u.hash(key)))
	if u.bkt[i_hash].get(hashed) { //there exists some elements with hash i_hash; check if key already exists.
		for i0 := u.mod(i_hash + int(u.bkt[i_hash].hashOS)); ; i0 = u.mod(i0 + int(u.bkt[i0].linkOS)) { //find i_hash+hashOS: start of the chain
			if u.bkt[i0].key == key {
				u.bkt[i0].val = val
				return
			}
				...
				if step < int(u.H) { //within H. we insert it here
					...
					for {
						if i_new := u.moveAndPut(i_empty); i_new == i_empty {
							break search
						} else if u.mod(i_new-i_hash) < int(u.H) {
