...
type PoolMap[K Maps.Hashable, V any] struct {
...
func MakePoolMap[K Maps.Hashable, V any](minBucketLen, maxBucketLen byte, maxHash uint) *PoolMap[K, V] {
	M := new(PoolMap[K, V])
...
func (u *PoolMap[K, V]) rehash(k K) uint {
...
func (u *PoolMap[K, V]) findHash(hash uint) *node[K] {
	s := *u.buckets.Load()
	return s[hash/((u.maxHash+1)/uint(len(s)))]
}

func (u *PoolMap[K, V]) Store(key K, val V) {
...
func (u *PoolMap[K, V]) Delete(key K) {
	u.LoadAndDelete(key)
}

func (u *PoolMap[K, V]) Load(key K) (val V, ok bool) {
	hash := u.rehash(key)
...
func (u *PoolMap[K, V]) HasKey(key K) bool {
	hash := u.rehash(key)
	_, _, _, _, f := u.findHash(hash).searchKey(key, hash)
	return f
}

func (u *PoolMap[K, V]) LoadOrStore(key K, val V) (V, bool) {
	for hash, vPtr := u.rehash(key), unsafe.Pointer(&val); ; {
		if l, ls, lsp, r, f := u.findHash(hash).searchKey(key, hash); f {
...
func (u *PoolMap[K, V]) LoadAndDelete(key K) (val V, loaded bool) {
	hash := u.rehash(key)
	if _, _, _, r, f := u.findHash(hash).searchKey(key, hash); f {
		loaded = r.delete()
		if loaded {
...
func (u *PoolMap[K, V]) Size() uint {
	return uint(u.size.Load())
}

func (u *PoolMap[K, V]) Take() (K, V) {
	t, _, _ := u.findHash(0).next()
	return t.k, *(*V)(t.getVPtr())
}

func (u *PoolMap[K, V]) Range(f func(K, V) bool) {
...
func (u *PoolMap[K, V]) LoadPtr(key K) (vp *V) {
	hash := u.rehash(key)
	if _, _, _, r, f := u.findHash(hash).searchKey(key, hash); f {
		vp = (*V)(r.getVPtr())
	}
